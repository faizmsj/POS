    ...
    <div class="relative min-h-screen overflow-hidden">
        ...
        <div class="relative mx-auto flex min-h-screen max-w-[1680px] gap-6 px-3 py-3 sm:px-4 lg:px-6">
                                ...
                                <nav class="mt-2 space-y-1.5">
                                    @foreach ($group['items'] as $item)
                                        ...
                                        <a href="{{ route($item['route']) }}"
                                            class="pos-nav-link {{ $active ? 'is-active' : '' }}">
            ...
            <div class="flex min-w-0 flex-1 flex-col">
                <header class="rounded-[30px] border border-white/70 bg-white/78 px-4 py-4 shadow-[0_16px_45px_rgba(15,23,42,0.08)] backdrop-blur sm:px-6">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            <div class="text-sm font-medium text-blue-600">Aplikasi Point of Sale</div>
                            <h1 class="mt-1 text-2xl font-semibold tracking-tight text-slate-950">{{ $title ?? 'POS Multicabang - Multicashier' }}</h1>
                            <p class="mt-1 text-sm text-slate-500">Panel operasional untuk cabang, kasir, FIFO inventory, PPOB, dan loyalty.</p>
                        </div>
                ...
                <main class="flex-1 px-1 py-6 sm:px-2">
                    @include('partials.flash')
                    {{ $slot ?? '' }}
                </main>

                <nav class="sticky bottom-3 z-10 mt-auto grid grid-cols-4 gap-2 rounded-[26px] border border-white/70 bg-white/92 p-2 shadow-[0_16px_45px_rgba(15,23,42,0.1)] backdrop-blur xl:hidden">
                    ...
                    <a href="{{ route('settings.index') }}" class="pos-mobile-link {{ request()->routeIs('settings.*') ? 'is-active' : '' }}">Setting</a>
                </nav>
            </div>
        </div>
    </div>
